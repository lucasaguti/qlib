...
### D014 — Implement the frozen evaluation as paired development-safe walk-forward units

- Date: 2026-09-26
- Status: accepted
- Scope: model | evaluation
- Decision: Implement `EVAL-PROTOCOL-001` as deterministic expanding-window units that admit labels only when matured by each issuance, refit population-standard-deviation scaling in every inner and outer training fold, tune the fixed Ridge grid with expanding one-origin-ahead validation, and preserve common eight-feature eligibility for the full model and all ablations. Report out-of-sample R-squared against both zero return and the matured expanding mean. Define directional hits by strict shared sign, with an exact-zero prediction matching only an exact-zero target. Define temporal stability as two fixed chronological halves with `floor(N/2)` observations in the first half. Use paired circular moving-block bootstrap rows, HAC(2) mean-loss tests, and one Holm family for the eight independently retuned leave-one-feature-out models. The default artifact runner is development-only and filters at the Arrow scan boundary through the frozen cutoff.
- Rationale: These choices make the protocol's metric, direction, and temporal-stability requirements executable without consulting the isolated final block. Strict-zero direction treatment does not recast the zero-return benchmark as an automatic positive forecast, and fixed halves avoid outcome-selected subperiods. Keeping ablation training eligibility fixed to all eight features preserves paired comparisons.
